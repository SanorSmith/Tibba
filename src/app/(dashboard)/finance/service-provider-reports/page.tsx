...
export default function ServiceProviderReports() {
  ...
  const fetchProviderReports = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/service-provider-reports');
    ...
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <Card className="p-4">
          ...
          <div className="text-lg font-bold text-emerald-600">{totalPaidAmount.toLocaleString()} IQD</div>
        </Card>
        <Card className="p-4">
          <div className="text-xs text-gray-500 mb-1">Total Pending</div>
          <div className="text-lg font-bold text-amber-600">{totalPendingAmount.toLocaleString()} IQD</div>
        </Card>
        <Card className="p-4">
                ...
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-4">
                    <div className="text-center p-3 bg-gray-50 rounded">
                      ...
                      <div className="text-lg font-bold text-gray-900">{provider.total_amount.toLocaleString()} IQD</div>
                    </div>
                    <div className="text-center p-3 bg-emerald-50 rounded">
                      <div className="text-xs text-emerald-600">Paid Amount</div>
                      <div className="text-lg font-bold text-emerald-600">{provider.paid_amount.toLocaleString()} IQD</div>
                    </div>
                    <div className="text-center p-3 bg-amber-50 rounded">
                      <div className="text-xs text-amber-600">Pending Amount</div>
                      <div className="text-lg font-bold text-amber-600">{provider.pending_amount.toLocaleString()} IQD</div>
                    </div>
                  </div>
                ...
                <tbody className="divide-y">
                  {filteredPayments.map(payment => (
                    <tr key={payment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-right">{payment.service_fee.toLocaleString()} IQD</td>
                      <td className="px-4 py-2 text-right font-semibold">{payment.total_fee.toLocaleString()} IQD</td>
