...
export default function StakeholdersPage() {
  ...
  const handleSave = () => {
    if (!current?.name_ar || !current?.mobile || !current?.role) { toast.error('Fill required fields'); return; }
    if (modal === 'create') financeStore.addStakeholder(current);
    else financeStore.updateStakeholder(current.stakeholder_id, current);
    toast.success(modal === 'create' ? 'Stakeholder created' : 'Stakeholder updated');
    setModal(null); reload();
  };

  const handleDelete = () => {
    if (deleteId) financeStore.deleteStakeholder(deleteId);
          ...
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Stakeholder ID</label>
                  <input
                ...
                <div>
                  ...
                  <select
                    value={current.role}
                    onChange={e => setCurrent({...current, role: e.target.value as StakeholderRole})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                  >
                    {roles.map(r => <option key={r} value={r}>{roleLabel[r]}</option>)}
                  </select>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setCurrent({...current, mobile: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Specialty (Arabic)</label>
                  <input
                    value={current.specialty_ar || ''}
                    onChange={e => setCurrent({...current, specialty_ar: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setCurrent({...current, license_number: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={current.service_type || ''}
                    onChange={e => setCurrent({...current, service_type: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                  >
                    ...
                    <option value="LAB_SERVICES">Laboratory Services</option>
                ...
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Bank Name (Arabic)</label>
                  <input
                    value={current.bank_name_ar || ''}
                    onChange={e => setCurrent({...current, bank_name_ar: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setCurrent({...current, account_number: e.target.value})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setCurrent({...current, is_active: e.target.value === 'ACTIVE'})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={current.default_share_type}
                    onChange={e => setCurrent({...current, default_share_type: e.target.value as 'PERCENTAGE' | 'FIXED_AMOUNT'})}
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                  >
                ...
                <div>
                  <label className="text-xs text-gray-500 block mb-1">
                    {current.default_share_type === 'PERCENTAGE' ? 'Default Share %' : 'Default Share Amount (IQD)'}
                  </label>
                  <input
                    type="number"
                    step={current.default_share_type === 'PERCENTAGE' ? '0.01' : '1000'}
                    value={current.default_share_type === 'PERCENTAGE' ? (current.default_share_percentage || 0) : (current.default_share_amount || 0)}
                    ...
                    className="w-full border rounded-lg px-3 py-2 text-sm"
                    placeholder={current.default_share_type === 'PERCENTAGE' ? 'e.g., 15' : 'e.g., 50000'}
                  />
                </div>
              </div>
            </div>
            <div className="p-4 border-t flex gap-2 justify-end">
              ...
              <button onClick={handleSave} className="bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium">
                {modal === 'create' ? 'Create Stakeholder' : 'Save Changes'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b">
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${roleColor(current.role)}`}>{roleLabel[current.role]}</span>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <h3 className="font-semibold text-sm mb-3 text-gray-700">Personal Information</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><span className="text-gray-500 block text-xs">Name (English)</span>{current.name_en||'-'}</div>
                  ...
                  <div><span className="text-gray-500 block text-xs">License Number</span>{current.license_number||'-'}</div>
                </div>
              </div>

              {/* Service Information */}
              <div>
                <h3 className="font-semibold text-sm mb-3 text-gray-700">Service Information</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><span className="text-gray-500 block text-xs">Service Type</span>
                    <span className="inline-block px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-medium">
                      {current.service_type ? current.service_type.replace('_', ' ') : 'Not specified'}
                    </span>
                  </div>
                  <div><span className="text-gray-500 block text-xs">Role</span>{roleLabel[current.role]}</div>
                </div>
              </div>

              {/* Financial Information */}
              <div>
                <h3 className="font-semibold text-sm mb-3 text-gray-700">Financial Information</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div><span className="text-gray-500 block text-xs">Share Type</span>
                    <span className="inline-block px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-medium">
                      {current.default_share_type}
                    </span>
                  </div>
                  <div><span className="text-gray-500 block text-xs">Default Share</span>
                    {current.default_share_type === 'PERCENTAGE' 
                      ? `${current.default_share_percentage || 0}%`
                      : `${fmt(current.default_share_amount || 0)} IQD`
                    }
                  </div>
                  <div><span className="text-gray-500 block text-xs">Bank Name</span>{current.bank_name_ar||'-'}</div>
                  <div><span className="text-gray-500 block text-xs">Account Number</span>{current.account_number||'-'}</div>
                  <div><span className="text-gray-500 block text-xs">IBAN</span>{current.iban||'-'}</div>
                </div>
              </div>

              {/* Share History */}
              <div>
                <h3 className="font-semibold text-sm mb-3 text-gray-700">Revenue Share History</h3>
