  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      {(modal === 'create' || modal === 'edit') && current && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b flex justify-between"><h2 className="modal-title">{modal === 'create' ? 'Add Stakeholder' : 'Edit Stakeholder'}</h2><button onClick={() => setModal(null)}><X size={18} /></button></div>
      ...
      {modal === 'view' && current && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b"><h2 className="modal-title">{current.name_ar}</h2><div className="flex items-center gap-2 mt-1"><span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${roleColor(current.role)}`}>{roleLabel[current.role]}</span><span className="text-xs text-gray-400">{current.stakeholder_code}</span></div></div>
