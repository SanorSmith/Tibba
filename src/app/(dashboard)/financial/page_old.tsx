...
// FORCE REBUILD - CACHE BREAKER VERSION 2.0
  ...
  const renderIncomeStatement = () => {
    ...
    return (
      <div className="max-w-2xl space-y-4">
        ...
        {/* Revenue Section - UPDATED VERSION 2.0 */}
        <Card>
          ...
          <CardContent className="p-4">
            {financialData.revenue.breakdown.map((item, index) => (
              // Use frontend label mapping as fallback - UPDATED
        ...
        <Card>
          ...
          <CardContent className="p-4">
            {financialData.expenses.breakdown.map((item, index) => (
              // Use frontend label mapping as fallback
              <div key={index} className="flex justify-between py-1.5 text-sm">
                <span className="ml-6 text-gray-600">
                  {item.category_label || item.display_name || getCategoryLabel(item.category) || item.category}
                </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-500 mt-1">Period: {financialData.summary.period}</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${financialData.summary.netIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {formatCurrency(financialData.summary.netIncome)}
              </div>
              <p className="text-xs text-gray-500 mt-1">
                Margin: {financialData.summary.profitMargin.toFixed(1)}%
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {financialData.summary.departmentId === 'all' ? 'All' : 
                 financialData.departments.find(d => d.department_id === financialData.summary.departmentId)?.department_name || 'Unknown'}
              </div>
              <p className="text-xs text-gray-500 mt-1">
                {financialData.metadata.usingNewTables ? 'Live Data' : 'Sample Data'}
              </p>
            </CardContent>
          </Card>
        </div>
