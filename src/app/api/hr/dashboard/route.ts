...
export async function GET() {
  ...
  const attendance = await safe(async () => {
    const r = await pool.query(`
      ...
      FROM daily_attendance WHERE date = CURRENT_DATE`);
    return r.rows[0];
  }, { present: 0, absent: 0 });

  const pendingLeaves = await safe(async () => {
    const r = await pool.query(`SELECT COUNT(*) AS c FROM leave_requests WHERE status ILIKE '%pending%'`);
  ...
  const recentApprovedLeaves = await safe(async () => {
    const r = await pool.query(`
      ...
      ORDER BY approved_at DESC NULLS LAST LIMIT 2`);
    return r.rows;
  }, []);
  ...
  const vacancies = await safe(async () => {
    const r = await pool.query(`
      ...
      FROM job_vacancies WHERE status = 'OPEN'`);
    return r.rows[0];
  }, { open_count: 0, openings: 0 });

  const candidates = await safe(async () => {
    const r = await pool.query(`SELECT COUNT(*) AS c FROM job_applications`);
    return parseInt(r.rows[0].c) || 0;
  }, 0);
  ...
  const payroll = await safe(async () => {
    const r = await pool.query(`
      ...
      FROM payroll_periods ORDER BY start_date DESC NULLS LAST LIMIT 1`);
    return r.rows[0] || null;
  }, null);

  const pendingReviews = await safe(async () => {
    const r = await pool.query(`SELECT COUNT(*) AS c FROM performance_reviews WHERE status = 'SUBMITTED'`);
    return parseInt(r.rows[0].c) || 0;
  }, 0);
  ...
  const recognitions = await safe(async () => {
    const r = await pool.query(`
      ...
      ORDER BY er.recognition_date DESC NULLS LAST LIMIT 3`);
    return r.rows;
  }, []);
