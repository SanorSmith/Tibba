...
export default function ManageGlobalDrugsPage() {
  ...
  const loadDrugs = async () => {
    ...
    setMessage("");
    try {
      const res = await fetch("/api/admin/manage-global-drugs");
  ...
  const deleteSelected = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedDrugIds.size} global drug records? This cannot be undone!`)) {
      return;
    }
  ...
  return (
    <div style={{ padding: 40, maxWidth: 1400, margin: "0 auto" }}>
      ...
      <p style={{ color: "#6b7280", marginBottom: 8 }}>
        View, rename, and delete drugs from the global drugs database.
      </p>
      
      {totalDrugs > 0 && (
        <div style={{ display: "flex", gap: 16, marginBottom: 24 }}>
          <div style={{ padding: "12px 16px", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8 }}>
            <div style={{ fontSize: 11, color: "#6b7280", marginBottom: 4 }}>Total Drugs</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: "#1e40af" }}>{totalDrugs.toLocaleString()}</div>
          </div>
          <div style={{ padding: "12px 16px", background: "#fef3c7", border: "1px solid #fcd34d", borderRadius: 8 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: "#92400e" }}>{duplicateCount.toLocaleString()}</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
        ...
        >
          {loading ? "Loading..." : "Load Global Drugs"}
        </button>
