      ...
      {loading ? (
        ...
        <>
          <Card className="flex-shrink-0">
            <CardHeader className="py-2 px-3 border-b"><CardTitle className="text-sm font-semibold">{openShift ? "Close shift" : "Open shift"}</CardTitle></CardHeader>
            <CardContent className="p-3">
              {openShift ? (
                <div className="space-y-2">
                  <div className="grid grid-cols-4 gap-2 text-sm">
                    ...
                    <div><div className="text-xs text-muted-foreground">Expected in drawer</div>
                      <span className="font-semibold">{money(Number(openShift.openingcash) + Number(openShift.cashCollected))}</span></div>
                  </div>
                  <div className="flex gap-2 items-end">
                    <div className="w-40"><label className="text-xs text-muted-foreground">Counted cash</label>
                      <Input type="number" value={actualCash} onChange={(e) => setActualCash(e.target.value)} /></div>
                    <div className="flex-1"><label className="text-xs text-muted-foreground">Variance reason (if any)</label>
                      <Input value={varianceReason} onChange={(e) => setVarianceReason(e.target.value)} /></div>
                    <Button onClick={closeShift} disabled={busy || actualCash === ""} className="gap-1"><Lock className="h-4 w-4" /> Close Shift</Button>
                  </div>
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="flex gap-2 items-end">
                  <div className="w-40"><label className="text-xs text-muted-foreground">Opening float</label>
                    <Input type="number" value={openingCash} onChange={(e) => setOpeningCash(e.target.value)} /></div>
                  <Button onClick={openNewShift} disabled={busy} className="gap-1"><LockOpen className="h-4 w-4" /> Open Shift</Button>
                </div>
              )}
            </CardContent>
          </Card>
          <Card className="flex-1 min-h-0 flex flex-col">
            <CardHeader className="py-2 px-3 border-b flex-shrink-0"><CardTitle className="text-sm font-semibold">Shift History</CardTitle></CardHeader>
            <CardContent className="p-0 flex-1 min-h-0 overflow-auto">
              {shifts.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-white border-b">
                    <tr className="text-left text-xs text-muted-foreground">
                      ...
                      <th className="px-3 py-2 text-right">Txns</th><th className="px-3 py-2 text-right">Collected</th><th className="px-3 py-2 text-right">Variance</th>
                    </tr>
                  </thead>
                  <tbody>
                    {shifts.map((s) => {
                      ...
                      return (
                        <tr key={s.id} className="border-b last:border-0 hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2"><Badge className={s.status === "OPEN" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}>{s.status}</Badge></td>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Card>
                  ...
                  <CardContent className="p-3">
                    {reports.collected.byMethod.length === 0 ? <div className="text-sm text-muted-foreground">Nothing collected yet.</div>
                ...
                <Card>
                  ...
                  <CardContent className="p-3">
                    {reports.byCashier.length === 0 ? <div className="text-sm text-muted-foreground">No collections yet.</div>
              ...
              <Card>
                ...
                <CardContent className="p-0">
                  {reports.topTests.length === 0 ? <div className="p-3 text-sm text-muted-foreground">Nothing billed yet.</div> : (
                    <table className="w-full text-sm">
                      ...
                      <tbody>{reports.topTests.map((t) => (
                        <tr key={t.name} className="border-b last:border-0">
                          <td className="px-3 py-2">{t.name}</td><td className="px-3 py-2 text-right">{t.count}</td>
              ...
              <Card>
                ...
                <CardContent className="p-0">
                  {reports.daily.length === 0 ? <div className="p-3 text-sm text-muted-foreground">No activity in this period.</div> : (
                    <table className="w-full text-sm">
                      <thead className="border-b"><tr className="text-left text-xs text-muted-foreground">
                      ...
                      <tbody>{reports.daily.map((d) => (
                        <tr key={d.day} className="border-b last:border-0">
                          <td className="px-3 py-2">{d.day}</td><td className="px-3 py-2 text-right">{money(d.invoiced)}</td>
