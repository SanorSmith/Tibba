...
export async function GET(
  ...
  try {
    ...
    if (patientid) {
      // The patient record and their insurance are facility-scoped, so they
      // are read under a tenant — which is then released before EHRbase is
      ...
      // Orders and their items are facility-scoped again, so they get their
      // own short transaction now that EHRbase is done with.
