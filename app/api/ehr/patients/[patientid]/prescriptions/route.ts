...
export async function POST(
  ...
  try {
    ...
    console.log("[POST /prescriptions] Received body:", JSON.stringify(body, null, 2));
    ...
    console.log("[POST /prescriptions] Prescriptions array length:", prescriptions.length);
    
    if (prescriptions.length === 0) {
      console.log("[POST /prescriptions] No prescriptions provided");
      return NextResponse.json(
    ...
    console.log("[POST /prescriptions] Using EHR ID:", ehrId);
    ...
    return NextResponse.json(
      { error: "Failed to create prescriptions in OpenEHR" },
      { status: 500 }
    );
  }
}
