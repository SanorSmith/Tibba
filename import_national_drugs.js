...
function extractCategory(nationalCode) {
  ...
  if (parts.length >= 2) {
    ...
    // Map category codes to names (you can expand this)
    const categoryMap = {
    ...
    await new Promise((resolve, reject) => {
      fs.createReadStream(csvPath)
        ...
        .on('data', (row) => {
          ...
          if (!rowArray[0] || !rowArray[2] || !rowArray[3]) return;
          if (rowArray[0] === '' || isNaN(rowArray[0])) return;
