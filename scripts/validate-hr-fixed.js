...
/**
 * HR MIGRATION VALIDATION - FIXED VERSION
 * Uses Supabase client to validate migration data
 * 
 * Usage: npm run validate:hr:fixed
 */
  ...
  try {
    ...
    if (!deptError && deptStats) {
      ...
      deptStats.forEach(record => {
        const deptName = (record.departments && record.departments.name) || 'Unknown';
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('🔍 HR MIGRATION VALIDATION (FIXED)');
