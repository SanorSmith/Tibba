/**
 * Clean HR Demo Data - Final Working Version
 * Removes demo data and leaves only 1 record per table
 */
...
async function cleanupTable(tableName, primaryKey = 'id') {
  try {
    ...
    const firstRecordId = firstRecord[0][primaryKey];
    
    await sql.unsafe(`
      DELETE FROM ${tableName} 
      WHERE ${sql.unsafe(primaryKey)} != ${firstRecordId}
    `);
...
async function cleanupHRData() {
  console.log('🧹 Starting HR Demo Data Cleanup (Final)...\n');

  const results = [];
  
  // Tables to clean with their primary keys
  const tablesToClean = [
    { name: 'departments', pk: 'id' },     // 7 records
    { name: 'staff', pk: 'id' },           // 9 records  
    { name: 'patients', pk: 'id' },        // 64 records
    { name: 'appointments', pk: 'id' },    // 13 records
    { name: 'users', pk: 'id' },            // 13 records
    { name: 'todos', pk: 'id' },            // 6 records
    { name: 'workspaces', pk: 'id' },      // 6 records
    { name: 'workspaceusers', pk: 'id' },   // 16 records
  ];
