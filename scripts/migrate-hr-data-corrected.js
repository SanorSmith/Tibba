...
const DATA_DIR = path.join(process.cwd(), 'src/data/hr');
...
async function batchInsert(table, records, stats) {
  const batches = [];
...
// ============================================================================
// DATA TRANSFORMATION FUNCTIONS (CORRECTED FOR ACTUAL SCHEMA)
// ============================================================================

function transformDepartment(dept, orgId) {
  return {
    id: generateUUID(),
...
function transformEmployee(emp, orgId, deptIdMap) {
...
function transformAttendanceRecord(record, empIdMap, orgId) {
...
function transformLeaveRequest(leave, empIdMap, orgId) {
  ...
  return {
    ...
    request_number: `LR-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    ...
    approver_1_id: null,
    approver_1_status: null,
    approver_1_date: null,
    approver_1_remarks: null,
    approver_2_id: null,
    approver_2_status: null,
    approver_2_date: null,
    approver_2_remarks: null,
    approver_3_id: null,
    approver_3_status: null,
    approver_3_date: null,
    approver_3_remarks: null,
    rejected_by: null,
    rejected_at: null,
...
function transformPayrollTransaction(payroll, empIdMap) {
...
async function migrateDepartments(orgId) {
  const stats = {
    table: 'departments',
    jsonCount: 0,
    inserted: 0,
...
async function migrateEmployees(orgId, deptIdMap) {
  const stats = {
    table: 'employees',
    jsonCount: 0,
    inserted: 0,
...
async function migrateAttendance(orgId, empIdMap) {
  const stats = {
  ...
  const transformed = records
    .map((record) => transformAttendanceRecord(record, empIdMap, orgId))
    .filter((r) => r !== null);
...
async function migrateLeaves(orgId, empIdMap) {
  const stats = {
  ...
  const transformed = leaves
    .map((leave) => transformLeaveRequest(leave, empIdMap, orgId))
    .filter((l) => l !== null);
...
async function migratePayroll(empIdMap) {
  const stats = {
  ...
  const transactions = [];
  jsonData.payroll_periods.forEach((period) => {
  ...
  const transformed = transactions
    .map((txn) => transformPayrollTransaction(txn, empIdMap))
    .filter((t) => t !== null);
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('🏥 TIBBNA HOSPITAL - HR DATA MIGRATION (CORRECTED)');
  console.log('='.repeat(80) + '\n');
