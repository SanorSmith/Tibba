/**
 * Clean HR Demo Data Script - Simple Version
 * Removes all demo data from HR models and leaves only 1 demo record per table
 * Uses the same database connection as the existing API routes
 */
...
const databaseUrl = process.env.DATABASE_URL || 
                   process.env.TIBBNA_DATABASE_URL ||
                   process.env.NEXT_PUBLIC_SUPABASE_URL?.replace('https://', 'postgresql://') + '?sslmode=require';
...
async function testConnection() {
  const connections = [
    ...
    { name: 'TIBBNA_DATABASE_URL', url: process.env.TIBBNA_DATABASE_URL },
    { name: 'Supabase (converted)', url: process.env.NEXT_PUBLIC_SUPABASE_URL?.replace('https://', 'postgresql://') + '?sslmode=require' }
  ];
