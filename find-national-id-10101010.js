...
async function findByNationalId10101010() {
  try {
    ...
    console.log('📋 PARTIAL MATCH RESULTS (contains 10101010):');
    console.log('===============================================');
    if (partialMatches.length > 0) {
      ...
      console.log('❌ No partial matches found containing "10101010"');
    }
    console.log('');

    // Search for similar patterns (with dashes or other formatting)
    const similarMatches = await sql`
      ...
      FROM patients 
      WHERE workspaceid = ${workspaceId}
      AND (nationalid = '10101010-0000' OR nationalid ILIKE '%10101010-%')
      ORDER BY createdat DESC
    `;

    console.log('📋 SIMILAR PATTERN RESULTS (with formatting):');
    console.log('===============================================');
    if (similarMatches.length > 0) {
      ...
      console.log('❌ No matches found for patterns like "10101010-xxxx"');
    }
    console.log('');

    // Check if this patient would appear in web app dashboard (has phone?)
    if (exactMatch.length > 0) {
      console.log('🌐 AVAILABILITY IN BOTH SYSTEMS:');
      console.log('==================================');
      exactMatch.forEach((patient, i) => {
